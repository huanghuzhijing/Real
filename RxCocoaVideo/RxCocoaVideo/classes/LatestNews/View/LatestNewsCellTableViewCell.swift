//
//  LatestNewsCellTableViewCell.swift
//  RxCocoaVideo
//
//  Created by 胡涛 on 2018/7/30.
...
class LatestNewsCellTableViewCell: UITableViewCell, RegisterCellFromNib {
    var didSelectCell: ((_ selectedIndex: Int)->())?
    var disposeBag = DisposeBag()
    var aNews = NewsModel(){
        didSet {
            avatarImageView.kf.setImage(with: URL(string: aNews.user_info.avatar_url))
            vImageView.isHidden = aNews.user_info.user_verified
            nameLabel.text = aNews.user_info.name
            timeAndDescriptionLabel.text = aNews.createTime + (aNews.user_info.verified_content != "" ? ("· \(aNews.user_info.verified_content)") : "")
            likeButton.setTitle(aNews.digg_count == 0 ? "zan" : aNews.diggCount, for: .normal)
            likeButton.isSelected = aNews.user_digg
            commentButton.setTitle(aNews.commentCount, for: .normal)
            forwardButton.setTitle(aNews.forwardCount, for: .normal)
            concernButton.isSelected = aNews.user_info.is_following
            areaLabel.text = (aNews.position.position != "" ? aNews.position.position + "· " : "") + aNews.readCount + "reads" + (aNews.brand_info != "" ? aNews.brand_info : "")
            contentLabel.attributedText =  NSMutableAttributedString(string: aNews.abstract)
            ...
            if aNews.image_list.count != 0{
                ...
                collectionView.thumbImages = aNews.image_list
                let c = aNews.large_image_list
                for i in c {
                    print(i.urlString)
                }
                collectionView.largeImages = aNews.image_list
